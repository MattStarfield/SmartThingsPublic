...
metadata {
	definition (name: "Andluca Smart Window DTH", namespace: "mattstarfield", author: "Matt Starfield", ocfDeviceType: "oic.d.switch", runLocally: true, minHubCoreVersion: '000.019.00012', executeCommandsLocally: true, genericHandler: "Zigbee") {
		capability "Actuator"
		...
		// Andluca
        fingerprint profileId: "0104", inClusters: "0000, 0003, 0004, 0005, 0006", deviceJoinName: "Andluca Smart Window" 
/*        
        // Generic
...
/* 
// Generic Parse & Event Method from SmartThings documentation
// https://docs.smartthings.com/en/latest/device-type-developers-guide/parse.html#overview

def parse(String description) {										// parse() is fed incoming raw message (description) from device
    log.debug "parse(description): $description"					// print incoming message (description) to IDE Live Logging

																	// local variables to hold attribute Name and Value
    def attrName = null
    def attrValue = null

    if (description?.startsWith("on/off:")) {						// look for defined start of string (just an example, not standardized)
        log.debug "switch command"									// print to Log that incoming string was matched
        attrName = "switch"											// set attribute Name (from Capabilites Reference)
        attrValue = description?.endsWith("1") ? "on" : "off"		// set the value of the end of string (just an example, not standardized)
    }

    def result = createEvent(name: attrName, value: attrValue)		// create a SmartThings Event (e.g. switch.on)

    log.debug "Parse returned ${result?.descriptionText}"			// print event descriptionText
    return result													// returning event fires it
    																// alternatively, use sendEvent() to both create and fire event
}
*/

// Parse device messages: from device to ST and from ST to device
// Custom Parsing Method - Matt Starfield 9/22/2020
def parse(String description) 
{
    def descMap = zigbee.parseDescriptionAsMap(description)						// parse message into map object
    log.debug descMap															// print map object to log
    
	def event = zigbee.getEvent(description)									// events from ST App to device
    
	if (event) 
    {        
        log.debug event.name + "." + event.value
        sendEvent(event)
	}
	else 
    {
        ...
        //def descMap = zigbee.parseDescriptionAsMap(description)						// parse incoming message into map object

        //log.debug descMap															// print map to log
        
        if (descMap && descMap.clusterInt == 0x0006 && descMap.commandInt == 0x01) 	// On/Off Cluster, "On" Command
        {
        	log.debug "switch.on"
            sendEvent(name: "switch", value: "on")
        }
        
        if (descMap && descMap.clusterInt == 0x0006 && descMap.commandInt == 0x00) 	// On/Off Cluster, "Off" Command
        {
